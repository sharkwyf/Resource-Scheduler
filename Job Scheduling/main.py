...
import random
data=[]
with open('task1_case1.txt','r') as f:
...
def check(limit,scheuding_now):
    ...
    if backtrace(arr, C_dict, limit,scheuding_now):
        return True
    else:
        return False

def backtrace(arr, groups, limit,scheuding_now):
    # 尝试每种可能性
    ...
    for start in start_list:
        ...
        for core_max in range(core_max_num,0,-1):
            ...
            if core_need_add>0:
                core_add = cores_copy[0:core_need_add]
            ...
            for index2, j in enumerate(job[0:-1]):
                ...
                scheuding_now_.append(key_use + "-" + str(dict_use[key_use]))
            ...
            if tp < tp_use:
                tp_use = tp
                core_use = core_now
                scheuding_now_use = [job[-1]]+scheuding_now_
        ...
        if start + tp_use <= limit:
            ...
            scheuding_now.append(scheuding_now_use)
            if backtrace(arr, groups_now, limit, scheuding_now):
                return True
            for i in core_use:
...
while l < r:
    ...
    print("start:"+str(mid))
    scheuding_now=[]
    if check(mid,scheuding_now):
        r = mid
        scheuding = scheuding_now
    else:
        l = mid + 1
print(l)
...
scheuding_dict = {}
for i in scheuding:
    scheuding_dict[i[0]] = i[1::]

scheuding_sorted = []
for x in range(len(B)):
    sorted_id2 = sorted(range(len(B[x])), key=lambda k: B[x][k],reverse=True)
    scheuding_sorted.append([scheuding_dict[x][y] for y in sorted_id2])

with open("result.txt","w") as f:                                                   #设置文件对象
    for i in scheuding_sorted:
        input=''
        for j in i:
            input += ' '+j
        input += '\n'
